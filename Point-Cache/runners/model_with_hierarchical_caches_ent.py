...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    f_ent = open(f'outputs/ent_acc_{args.lm3d}_{args.dataset}.log', 'w')
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        # --- 1. zero shot
        zero_acc = cls_acc(clip_logits, target)
        zero_acc_list.append(zero_acc)
        
        f_ent.write(f'{i}\t{clip_logits.squeeze().cpu().numpy().tolist()}\t{sum(zero_acc_list)/len(zero_acc_list)}\t')
        
        final_logits = clip_logits.clone()
        if pos_enabled and pos_cache:
            final_logits += compute_cache_logits(pc_feats, pos_cache, pos_params['alpha'], pos_params['beta'], clip_weights)
            
            global_acc = cls_acc(final_logits, target)
            global_acc_list.append(global_acc)
            
            f_ent.write(f'{final_logits.squeeze().cpu().numpy().tolist()}\t{sum(global_acc_list)/len(global_acc_list)}\t')
        ...
        # 3. hierar cache
        hierar_acc = cls_acc(final_logits, target)
        hierar_acc_list.append(hierar_acc)
        
        f_ent.write(f'{final_logits.squeeze().cpu().numpy().tolist()}\t{sum(hierar_acc_list)/len(hierar_acc_list)}\n')
        
        if i % args.print_freq == 0:
            print("---- TDA's test accuracy: {:.2f}. ----\n".format(sum(hierar_acc_list)/len(hierar_acc_list)))
...
def main():
    ...
    run_test_tda(args, cfg['positive'], cfg['negative'], test_loader, lm3d_model, clip_weights)
