...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        zero_t_duration += zero_t - start
        global_t_duration += global_t - start
        hierar_t_duration += hierar_t - start
        
        acc = cls_acc(final_logits, target.cuda())  
        accuracies.append(acc)
        wandb.log({"zero_speed": (i+1) / zero_t_duration})
        wandb.log({"global_speed": (i+1) / global_t_duration})
        wandb.log({"hierar_speed": (i+1) / hierar_t_duration})
...
def main():
    ...
    if args.wandb:
        if args.lm3d == 'openshape':
            prefix = f"[speed]/{args.cache_type}_cache/{args.lm3d}-{args.oshape_version}" 
        elif args.lm3d == 'ulip':
            prefix = f"[speed]/{args.cache_type}_cache/{args.ulip_version}"
        else:
            prefix = f"[speed]/{args.cache_type}_cache/{args.lm3d}"
