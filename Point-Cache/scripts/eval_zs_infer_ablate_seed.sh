...
dataset=$4      # modelnet_c, sonn_c, snv2_c, omniobject3d, o-lvis, modelnet40, scanobjnn
...
seed=${12}      # 1, 2, 3 ... different seed for experiments
...
if [ "$lm3d" = "uni3d" ]; then
    ...
    pueue add -g point_cache_zs_infer \
    python runners/zs_infer_ablate_seed.py \
    --config configs \
    --wandb-log \
    --seed ${seed} \
    --lm3d ${lm3d} \
